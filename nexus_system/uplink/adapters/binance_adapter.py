    ...
    async def initialize(self, verbose: bool = False, **kwargs) -> bool:
        ...
        try:
            ...
            if not self._api_key or not self._api_secret:
                if verbose: print(f"❌ BinanceAdapter: Missing API credentials!")
                return False
            ...
            try:
                ...
                if verbose: print(f"✅ BinanceAdapter: Markets loaded (public endpoint OK)")
            ...
            try:
                ...
                if verbose: print(f"✅ BinanceAdapter: Auth OK (balance fetched)")
                ...
                try:
                    ...
                    if verbose: print(f"⚙️ BinanceAdapter: Position Mode -> {mode_str}")
                except Exception as mode_err:
                    if verbose: print(f"⚠️ BinanceAdapter: Could not detect Position Mode - {mode_err}")
                    # Keep default _is_hedge_mode = False

                return True
            except Exception as auth_err:
                err_str = str(auth_err)
                if verbose: print(f"❌ BinanceAdapter: Auth failed - {auth_err}")
